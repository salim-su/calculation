...
export class StartupService {
  constructor(
    ...
    iconSrv.fetchFromIconfont({
      scriptUrl: 'http://at.alicdn.com/t/font_2951351_zsruh329bqn.js',
    });
  }

  private viaHttp(resolve: any, reject: any) {
    if (location.href.includes('mobile/takeform')) {
      resolve(null);
      return;
    }
    zip(
      ...
      this.httpClient.get('blade-system/menu/routes?subsystemId=' + environment.subsystem.calculation.id),
      // this.httpClient.get('blade-system/deptBiz/deptBizByLoginUser?code=tiog'),
      // this.httpClient.get('blade-system/menu/routes?subsystemId'),
    )
      .pipe(
        catchError(([info, menus]) => {
          resolve(null);
          return [menus];
        }),
      )
      .subscribe(
        ([info, menus]) => {
          ...
          // this.settingService.setData('menus', menus);
          // console.log(this.settingService.getData('menus')[0]?.children[0]['path']);
          // environment.redirectTo = this.settingService.getData(menus);

          // console.log(this.arrayService.flat(menus));
          // console.log(this.arrayService.arrToTree(menus));
